...
export default function ProductReviews({ productId }) {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/products/${productId}/reviews?sortBy=${sortBy}&filterRating=${filterRating}`);
  ...
  const handleSubmitReview = async () => {
    try {
      ...
      setNewReview({ rating: 5, title: '', comment: '', name: '', verifiedPurchase: false });
  ...
  if (loading) {
    return (
      <div style={styles.container}>
        <div style={styles.loader}>Loading reviews...</div>
      </div>
    );
  }
              ...
              <div style={styles.reviewHeader}>
                <div style={styles.reviewerInfo}>
                  ...
                  <div style={styles.reviewerDetails}>
                    <div style={styles.reviewerName}>{review.name || 'Anonymous'}</div>
